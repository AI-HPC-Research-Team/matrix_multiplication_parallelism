...
from .mylayers import _LayerNorm, _Linear, _Dropout, _check_input_shape, \
...
from mindspore.nn.transformer.op_parallel_config import default_dpmp_config, _PipeLineConfig, OpParallelConfig, _Config, _check_config, \
    MoEParallelConfig
    ...
    def __init__(self, hidden_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            if expert_num > 1:
                ...
                self.mapping.shard(strategy_matmul=((dp, mp), (dp, mp)),
                                   strategy_bias=((dp, mp), (mp,)),
                                   strategy_activation=((dp, mp),))
            # Project back to hidden_size
            self.projection = _Linear(in_channels=output_size,
            ...
            if expert_num > 1:
                ...
                self.projection.shard(strategy_matmul=((dp, mp), (dp, mp)),
                                      strategy_bias=((dp, mp), (mp,)))
            self.projection.bias.parallel_optimizer = False
            self.dropout = _Dropout(1 - dropout_rate)
            self.dropout.shard(((dp, mp),))
            self.dropout_3d = _Dropout(1 - dropout_rate)
            # jinraol
            #self.dropout_3d.shard(((dp, 1, 1),))
            self.dropout_3d.shard(((dp, 1, mp),))
            self.dropout_4d = _Dropout(1 - dropout_rate)
            self.dropout_4d.shard(((dp, ep, mp, 1),))
            self.cast = P.Cast()
...
class MultiHeadAttention(Cell):
    ...
    def __init__(self, batch_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            # jinraol
            self.projection.shard(strategy_bias=((parallel_config.data_parallel, parallel_config.model_parallel), (parallel_config.model_parallel,)),
            ...
            # jinraol
            self.dropout = _Dropout(1 - hidden_dropout_rate).shard((parallel_config.data_parallel, 1, parallel_config.model_parallel))
            ...
            self.dropout.shard(((parallel_config.data_parallel, parallel_config.model_parallel),))
            self.prob_dropout = _Dropout(1 - attention_dropout_rate)
            ...
            self.dense1 = _Linear(hidden_size,
                                  ...
                                  param_init_type=param_init_type).to_float(compute_dtype)
            # jinraol
            self.dense1.shard(strategy_matmul=((parallel_config.data_parallel, parallel_config.model_parallel), (parallel_config.data_parallel, parallel_config.model_parallel)),
            ...
            self.dense2.shard(strategy_matmul=((parallel_config.data_parallel, parallel_config.model_parallel), (parallel_config.data_parallel, parallel_config.model_parallel)),
                              strategy_bias=((parallel_config.data_parallel, parallel_config.model_parallel),
                                             (parallel_config.model_parallel,)))
            # Value
            self.dense3 = _Linear(hidden_size,
            ...
            #                  strategy_bias=((parallel_config.data_parallel, parallel_config.model_parallel),
            #                                 (parallel_config.model_parallel,)))
            self.dense3.shard(strategy_matmul=((parallel_config.data_parallel, parallel_config.model_parallel), (parallel_config.data_parallel, parallel_config.model_parallel)),
                              strategy_bias=((parallel_config.data_parallel, parallel_config.model_parallel),
                                             (parallel_config.model_parallel,)))
            self.dtype = compute_dtype
            self.softmax_dtype = softmax_compute_type
...
class TransformerEncoderLayer(Cell):
    ...
    def __init__(self,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            self.layernorm1.shard(((parallel_config.data_parallel, parallel_config.model_parallel),))
            self.layernorm2 = _LayerNorm((hidden_size,)).to_float(layernorm_compute_type)
            # jinraol
            #self.layernorm2.shard(((parallel_config.data_parallel, 1),))
            self.layernorm2.shard(((parallel_config.data_parallel, parallel_config.model_parallel),))
            ...
            #self.add = P.Add().shard(((parallel_config.data_parallel, 1), (parallel_config.data_parallel, 1)))
            #self.add_3d = P.Add().shard(((parallel_config.data_parallel, 1, 1), (parallel_config.data_parallel, 1, 1)))
            self.add = P.Add().shard(((parallel_config.data_parallel, parallel_config.model_parallel), (parallel_config.data_parallel, parallel_config.model_parallel)))
            self.add_3d = P.Add().shard(((parallel_config.data_parallel, 1, parallel_config.model_parallel), (parallel_config.data_parallel, 1, parallel_config.model_parallel)))
            self.dtype = mstype.float16
            self.key_past = None
            self.value_past = None
