...
from .utils import _get_model_parallel_group
...
class _LayerNorm(Cell):
    ...
    def shard(self, strategy):
        ...
        if self.is_self_defined:
            ...
            dp = strategy[0][0]
            mp = strategy[0][1]
            self.mean.shard(((dp, mp), ))
            self.square.shard(((dp, mp), ))
            self.sqrt.shard(((dp, 1), ))
            self.sub1.shard(((dp, mp), (dp, 1)))
            self.sub2.shard(((dp, mp), (dp, 1)))
            self.add.shard(((dp, 1), ()))
            self.mul.shard(((dp, mp), (mp, )))
            self.add2.shard(((dp, mp), (mp, )))
            self.real_div.shard(((dp, mp), (dp, 1)))
        else:
