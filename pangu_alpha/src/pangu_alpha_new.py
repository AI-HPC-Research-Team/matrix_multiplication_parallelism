...
from .transformer import VocabEmbedding
from .transformer import TransformerEncoder, TransformerEncoderLayer, \
    AttentionMask
from mindspore.nn.transformer import MoEConfig
from .mylayers import _LayerNorm, _Dropout
...
class EmbeddingLayer(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.add = P.Add().shard(
            ((config.parallel_config.data_parallel, 1, config.parallel_config.model_parallel), (config.parallel_config.data_parallel, 1, config.parallel_config.model_parallel)))
        self.dropout = _Dropout(1 - config.dropout_rate)
        # jinraol
        #self.dropout.shard(((config.parallel_config.data_parallel, 1, 1),))
        self.dropout.shard(((config.parallel_config.data_parallel, 1, config.parallel_config.model_parallel),))
...
class PanGuHead(Cell):
    ...
    def __init__(self,
        ...
        if parallel_config.vocab_emb_dp:
            ...
            self.matmul = P.MatMul(transpose_b=True).shard(((1, 1), (1, 1)))
        else:
            # self.matmul = P.MatMul(transpose_b=True).shard(((parallel_config.data_parallel, 1), (parallel_config.model_parallel, 1)))
            self.matmul = P.MatMul(transpose_b=True).shard(((1, 1), (1, 1)))
...
class PanguAlpha_Model(Cell):
    ...
    def __init__(self, config):
        ...
        self.layernorm.shard(((config.parallel_config.data_parallel, config.parallel_config.model_parallel),))
